...
def get_embedding_model(model_choice: str):
    ...
    if model_choice not in _model_cache:
        if model_choice == EmbeddingModel.LOCAL_E5_MULTILINGUAL:
            ...
            _model_cache[model_choice] = SentenceTransformer("intfloat/multilingual-e5-large-instruct")
...
def generate_embeddings(texts: List[str], model_choice: str) -> List[List[float]]:
    ...
    if model_choice == EmbeddingModel.MOCK_MODEL: # Add this block
        ...
        return [[float(i+1) / 10.0] * get_embedding_dimensions(model_choice) for i, _ in enumerate(texts)]
        ...
        logger.info(f"Generating embeddings for {len(texts)} texts using E5 Multilingual model")
...
def generate_query_embedding(query_text: str, model_choice: str) -> List[float]:
