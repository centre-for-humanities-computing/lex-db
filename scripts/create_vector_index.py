...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create a vector index from a source table")
    ...
    parser.add_argument(
        ...
        "-i",
        required=True,
        help="Name for the new vector index"
    )
    ...
    try:
        ...
        logger.info(f"Creating vector index '{args.index_name}' using {args.embedding_model} model")
        create_vector_index(
            db_conn=db_conn,
