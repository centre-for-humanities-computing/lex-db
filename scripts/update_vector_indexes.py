...
def get_last_processed_timestamp(index_name: str) -> float:
    ...
    try:
        with open(state_file, "r") as f:
            state = json.load(f)
        return float(state.get("last_timestamp", 0.0))
    except (json.JSONDecodeError, FileNotFoundError):
        return 0.0


def set_last_processed_timestamp(index_name: str, timestamp: float) -> None:
...
def update_vector_index(
    ...
    logger.info(
        f"Updating vector index '{vector_index_name}' from source table '{source_table}'"
    )
    logger.info(f"Processing changes since timestamp: {last_timestamp}")

    stats = {"new_or_updated": 0, "deleted": 0, "errors": 0}
    ...
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name=?", (source_table,)
    )
    if not cursor.fetchone():
    ...
    try:
        ...
        raise ValueError(
            f"Column '{updated_at_column}' not found in table '{source_table}'"
        )
    ...
    try:
        ...
        for article in new_or_updated_articles:
            ...
            try:
                ...
                remove_article_from_vector_index(
                    db_conn, vector_index_name, article_rowid
                )
    ...
    try:
        ...
        for article_id_to_delete in ids_to_delete:
            try:
                remove_article_from_vector_index(
                    db_conn, vector_index_name, article_id_to_delete
                )
                stats["deleted"] += 1
            except Exception as e:
    ...
    if stats["errors"] == 0:
        ...
        logger.info(
            f"Updated timestamp for index '{vector_index_name}' to {current_timestamp}"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Update vector indexes with new content"
    )
